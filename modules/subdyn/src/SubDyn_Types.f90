...
! =========  SD_ParameterType  =======
  TYPE, PUBLIC :: SD_ParameterType
    ...
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FRIMMP      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FMIMMP      !< Component of right hand side due to seismic input signal [-]   
    ! AÑADO GIRO
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FRIMKP_PHI      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FMIMKP_PHI      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FRIMCP_PHI      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FMIMCP_PHI      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FRIMMP_PHI      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FMIMMP_PHI      !< Component of right hand side due to seismic input signal [-] 
    ! AÑADO VERTICAL
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FRIMKP_V      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FMIMKP_V      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FRIMCP_V      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FMIMCP_V      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FRIMMP_V      !< Component of right hand side due to seismic input signal [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FMIMMP_V      !< Component of right hand side due to seismic input signal [-]     
    ...
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: UgData      !< time, input ground displacement, velocity and acceleration, input ground twist, angular velocity and acceleration [-]
    ...
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: RRbase_PHI      !< Influence vector of ground shaking (base part of the vector) [-]
...
ENDIF
    ...
    DstMiscData%PHIg = SrcMiscData%PHIg
    DstMiscData%PHIdotg = SrcMiscData%PHIdotg
    DstMiscData%PHIddotg = SrcMiscData%PHIddotg
    DstMiscData%Vg = SrcMiscData%Vg
    DstMiscData%Vdotg = SrcMiscData%Vdotg
    DstMiscData%Vddotg = SrcMiscData%Vddotg    
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
      ...
      Re_BufSz   = Re_BufSz   + 1  ! PHIg
      Re_BufSz   = Re_BufSz   + 1  ! PHIdotg
      Re_BufSz   = Re_BufSz   + 1  ! PHIddotg   
      Re_BufSz   = Re_BufSz   + 1  ! Vg
      Re_BufSz   = Re_BufSz   + 1  ! Vdotg
      Re_BufSz   = Re_BufSz   + 1  ! Vddotg       
  ...
  END IF
      ...
      Re_Xferred   = Re_Xferred   + 1
      ReKiBuf ( Re_Xferred:Re_Xferred+(1)-1 ) = InData%PHIg
      Re_Xferred   = Re_Xferred   + 1
      ReKiBuf ( Re_Xferred:Re_Xferred+(1)-1 ) = InData%PHIdotg
      Re_Xferred   = Re_Xferred   + 1
      ReKiBuf ( Re_Xferred:Re_Xferred+(1)-1 ) = InData%PHIddotg
      Re_Xferred   = Re_Xferred   + 1 
      ReKiBuf ( Re_Xferred:Re_Xferred+(1)-1 ) = InData%Vg
      Re_Xferred   = Re_Xferred   + 1
      ReKiBuf ( Re_Xferred:Re_Xferred+(1)-1 ) = InData%Vdotg
      Re_Xferred   = Re_Xferred   + 1
      ReKiBuf ( Re_Xferred:Re_Xferred+(1)-1 ) = InData%Vddotg
      Re_Xferred   = Re_Xferred   + 1       
 ...
 ! buffers to store meshes, if any
  ...
  END IF
      ...
      Re_Xferred   = Re_Xferred + 1
      OutData%PHIg = ReKiBuf( Re_Xferred )
      Re_Xferred   = Re_Xferred + 1
      OutData%PHIdotg = ReKiBuf( Re_Xferred )
      Re_Xferred   = Re_Xferred + 1
      OutData%PHIddotg = ReKiBuf( Re_Xferred )
      Re_Xferred   = Re_Xferred + 1
      OutData%Vg = ReKiBuf( Re_Xferred )
      Re_Xferred   = Re_Xferred + 1
      OutData%Vdotg = ReKiBuf( Re_Xferred )
      Re_Xferred   = Re_Xferred + 1
      OutData%Vddotg = ReKiBuf( Re_Xferred )
      Re_Xferred   = Re_Xferred + 1      
...
END IF
IF (ALLOCATED(SrcParamData%FRIMMP)) THEN !AÑADO FM
  i1_l = LBOUND(SrcParamData%FRIMMP,1)
  i1_u = UBOUND(SrcParamData%FRIMMP,1)
  IF (.NOT. ALLOCATED(DstParamData%FRIMMP)) THEN 
    ALLOCATE(DstParamData%FRIMMP(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FRIMMP.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FRIMMP = SrcParamData%FRIMMP
ENDIF
IF (ALLOCATED(SrcParamData%FMIMMP)) THEN
  i1_l = LBOUND(SrcParamData%FMIMMP,1)
  i1_u = UBOUND(SrcParamData%FMIMMP,1)
  IF (.NOT. ALLOCATED(DstParamData%FMIMMP)) THEN 
    ALLOCATE(DstParamData%FMIMMP(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FMIMMP.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FMIMMP = SrcParamData%FMIMMP  
  END IF 
!AÑADIR GIRO   
IF (ALLOCATED(SrcParamData%FRIMKP_PHI)) THEN
  i1_l = LBOUND(SrcParamData%FRIMKP_PHI,1)
  i1_u = UBOUND(SrcParamData%FRIMKP_PHI,1)
  IF (.NOT. ALLOCATED(DstParamData%FRIMKP_PHI)) THEN 
    ALLOCATE(DstParamData%FRIMKP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FRIMKP_PHI.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FRIMKP_PHI = SrcParamData%FRIMKP_PHI
ENDIF
IF (ALLOCATED(SrcParamData%FMIMKP_PHI)) THEN
  i1_l = LBOUND(SrcParamData%FMIMKP_PHI,1)
  i1_u = UBOUND(SrcParamData%FMIMKP_PHI,1)
  IF (.NOT. ALLOCATED(DstParamData%FMIMKP_PHI)) THEN 
    ALLOCATE(DstParamData%FMIMKP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FMIMKP_PHI.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FMIMKP_PHI = SrcParamData%FMIMKP_PHI
ENDIF
IF (ALLOCATED(SrcParamData%FRIMCP_PHI)) THEN
  i1_l = LBOUND(SrcParamData%FRIMCP_PHI,1)
  i1_u = UBOUND(SrcParamData%FRIMCP_PHI,1)
  IF (.NOT. ALLOCATED(DstParamData%FRIMCP_PHI)) THEN 
    ALLOCATE(DstParamData%FRIMCP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FRIMCP_PHI.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FRIMCP_PHI = SrcParamData%FRIMCP_PHI
ENDIF
IF (ALLOCATED(SrcParamData%FMIMCP_PHI)) THEN
  i1_l = LBOUND(SrcParamData%FMIMCP_PHI,1)
  i1_u = UBOUND(SrcParamData%FMIMCP_PHI,1)
  IF (.NOT. ALLOCATED(DstParamData%FMIMCP_PHI)) THEN 
    ALLOCATE(DstParamData%FMIMCP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FMIMCP_PHI.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FMIMCP_PHI = SrcParamData%FMIMCP_PHI
END IF
IF (ALLOCATED(SrcParamData%FRIMMP_PHI)) THEN !AÑADO FM
  i1_l = LBOUND(SrcParamData%FRIMMP_PHI,1)
  i1_u = UBOUND(SrcParamData%FRIMMP_PHI,1)
  IF (.NOT. ALLOCATED(DstParamData%FRIMMP_PHI)) THEN 
    ALLOCATE(DstParamData%FRIMMP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FRIMMP_PHI.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FRIMMP_PHI = SrcParamData%FRIMMP_PHI
ENDIF
IF (ALLOCATED(SrcParamData%FMIMMP_PHI)) THEN
  i1_l = LBOUND(SrcParamData%FMIMMP_PHI,1)
  i1_u = UBOUND(SrcParamData%FMIMMP_PHI,1)
  IF (.NOT. ALLOCATED(DstParamData%FMIMMP_PHI)) THEN 
    ALLOCATE(DstParamData%FMIMMP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FMIMMP_PHI.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FMIMMP_PHI = SrcParamData%FMIMMP_PHI       
ENDIF
! AÑADO VERTICAL
IF (ALLOCATED(SrcParamData%FRIMKP_V)) THEN
  i1_l = LBOUND(SrcParamData%FRIMKP_V,1)
  i1_u = UBOUND(SrcParamData%FRIMKP_V,1)
  IF (.NOT. ALLOCATED(DstParamData%FRIMKP_V)) THEN 
    ALLOCATE(DstParamData%FRIMKP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FRIMKP_V.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FRIMKP_V = SrcParamData%FRIMKP_V
ENDIF
IF (ALLOCATED(SrcParamData%FMIMKP_V)) THEN
  i1_l = LBOUND(SrcParamData%FMIMKP_V,1)
  i1_u = UBOUND(SrcParamData%FMIMKP_V,1)
  IF (.NOT. ALLOCATED(DstParamData%FMIMKP_V)) THEN 
    ALLOCATE(DstParamData%FMIMKP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FMIMKP_V.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FMIMKP_V = SrcParamData%FMIMKP_V
ENDIF
IF (ALLOCATED(SrcParamData%FRIMCP_V)) THEN
  i1_l = LBOUND(SrcParamData%FRIMCP_V,1)
  i1_u = UBOUND(SrcParamData%FRIMCP_V,1)
  IF (.NOT. ALLOCATED(DstParamData%FRIMCP_V)) THEN 
    ALLOCATE(DstParamData%FRIMCP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FRIMCP_V.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FRIMCP_V = SrcParamData%FRIMCP_V
ENDIF
IF (ALLOCATED(SrcParamData%FMIMCP_V)) THEN
  i1_l = LBOUND(SrcParamData%FMIMCP_V,1)
  i1_u = UBOUND(SrcParamData%FMIMCP_V,1)
  IF (.NOT. ALLOCATED(DstParamData%FMIMCP_V)) THEN 
    ALLOCATE(DstParamData%FMIMCP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FMIMCP_V.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FMIMCP_V = SrcParamData%FMIMCP_V
END IF
IF (ALLOCATED(SrcParamData%FRIMMP_V)) THEN !AÑADO FM
  i1_l = LBOUND(SrcParamData%FRIMMP_V,1)
  i1_u = UBOUND(SrcParamData%FRIMMP_V,1)
  IF (.NOT. ALLOCATED(DstParamData%FRIMMP_V)) THEN 
    ALLOCATE(DstParamData%FRIMMP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FRIMMP_V.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FRIMMP_V = SrcParamData%FRIMMP_V
ENDIF
IF (ALLOCATED(SrcParamData%FMIMMP_V)) THEN
  i1_l = LBOUND(SrcParamData%FMIMMP_V,1)
  i1_u = UBOUND(SrcParamData%FMIMMP_V,1)
  IF (.NOT. ALLOCATED(DstParamData%FMIMMP_V)) THEN 
    ALLOCATE(DstParamData%FMIMMP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%FMIMMP_V.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%FMIMMP_V = SrcParamData%FMIMMP_V       
ENDIF
!TERMINO VERTICAL
...
ENDIF
IF (ALLOCATED(ParamData%FRIMMP)) THEN
  DEALLOCATE(ParamData%FRIMMP)
ENDIF
IF (ALLOCATED(ParamData%FMIMMP)) THEN
  DEALLOCATE(ParamData%FMIMMP)  
ENDIF
! AÑADIR GIRO
IF (ALLOCATED(ParamData%FRIMKP_PHI)) THEN
  DEALLOCATE(ParamData%FRIMKP_PHI)
ENDIF
IF (ALLOCATED(ParamData%FMIMKP_PHI)) THEN
  DEALLOCATE(ParamData%FMIMKP_PHI)
ENDIF
IF (ALLOCATED(ParamData%FRIMCP_PHI)) THEN
  DEALLOCATE(ParamData%FRIMCP_PHI)
ENDIF
IF (ALLOCATED(ParamData%FMIMCP_PHI)) THEN
  DEALLOCATE(ParamData%FMIMCP_PHI)
ENDIF
IF (ALLOCATED(ParamData%FRIMMP_PHI)) THEN
  DEALLOCATE(ParamData%FRIMMP_PHI)
ENDIF
IF (ALLOCATED(ParamData%FMIMMP_PHI)) THEN
  DEALLOCATE(ParamData%FMIMMP_PHI)  
ENDIF
!AÑADIR VERTICAL
IF (ALLOCATED(ParamData%FRIMKP_V)) THEN
  DEALLOCATE(ParamData%FRIMKP_V)
ENDIF
IF (ALLOCATED(ParamData%FMIMKP_V)) THEN
  DEALLOCATE(ParamData%FMIMKP_V)
ENDIF
IF (ALLOCATED(ParamData%FRIMCP_V)) THEN
  DEALLOCATE(ParamData%FRIMCP_V)
ENDIF
IF (ALLOCATED(ParamData%FMIMCP_V)) THEN
  DEALLOCATE(ParamData%FMIMCP_V)
ENDIF
IF (ALLOCATED(ParamData%FRIMMP_V)) THEN
  DEALLOCATE(ParamData%FRIMMP_V)
ENDIF
IF (ALLOCATED(ParamData%FMIMMP_V)) THEN
  DEALLOCATE(ParamData%FMIMMP_V)  
ENDIF
!TERMINO VERTICAL
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FRIMMP allocated yes/no
  IF ( ALLOCATED(InData%FRIMMP) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FRIMMP upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FRIMMP)  ! FRIMMP
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FMIMMP allocated yes/no
  IF ( ALLOCATED(InData%FMIMMP) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FMIMMP upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FMIMMP)  ! FMIMMP      
  END IF
  !AÑADIR GIRO
  Int_BufSz   = Int_BufSz   + 1     ! FRIMKP_PHI allocated yes/no
  IF ( ALLOCATED(InData%FRIMKP_PHI) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FRIMKP_PHI upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FRIMKP_PHI)  ! FRIMKP_PHI
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FMIMKP_PHI allocated yes/no
  IF ( ALLOCATED(InData%FMIMKP_PHI) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FMIMKP_PHI upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FMIMKP_PHI)  ! FMIMKP_PHI
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FRIMCP_PHI allocated yes/no
  IF ( ALLOCATED(InData%FRIMCP_PHI) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FRIMCP_PHI upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FRIMCP_PHI)  ! FRIMCP_PHI
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FMIMCP_PHI allocated yes/no
  IF ( ALLOCATED(InData%FMIMCP_PHI) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FMIMCP_PHI upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FMIMCP_PHI)  ! FMIMCP_PHI
  END IF
  Int_BufSz   = Int_BufSz   + 1 ! FRIMMP_PHI allocated yes/no 
  IF ( ALLOCATED(InData%FRIMMP_PHI) ) THEN 
    Int_BufSz   = Int_BufSz   + 2*1  ! FRIMMP_PHI upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FRIMMP_PHI)  ! FRIMMP_PHI
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FMIMMP_PHI allocated yes/no
  IF ( ALLOCATED(InData%FMIMMP_PHI) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FMIMMP_PHI upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FMIMMP_PHI)  ! FMIMMP_PHI      
  END IF
! AÑADIR VERTICAL
  Int_BufSz   = Int_BufSz   + 1     ! FRIMKP_V allocated yes/no
  IF ( ALLOCATED(InData%FRIMKP_V) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FRIMKP_V upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FRIMKP_V)  ! FRIMKP_V
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FMIMKP_V allocated yes/no
  IF ( ALLOCATED(InData%FMIMKP_V) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FMIMKP_V upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FMIMKP_V)  ! FMIMKP_V
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FRIMCP_V allocated yes/no
  IF ( ALLOCATED(InData%FRIMCP_V) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FRIMCP_V upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FRIMCP_V)  ! FRIMCP_V
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FMIMCP_V allocated yes/no
  IF ( ALLOCATED(InData%FMIMCP_V) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FMIMCP_V upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FMIMCP_V)  ! FMIMCP_V
  END IF
  Int_BufSz   = Int_BufSz   + 1 ! FRIMMP_V allocated yes/no 
  IF ( ALLOCATED(InData%FRIMMP_V) ) THEN 
    Int_BufSz   = Int_BufSz   + 2*1  ! FRIMMP_V upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FRIMMP_V)  ! FRIMMP_V
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FMIMMP_V allocated yes/no
  IF ( ALLOCATED(InData%FMIMMP_V) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FMIMMP_V upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%FMIMMP_V)  ! FMIMMP_V      
  END IF
!TERMINO VERTICAL
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! RRbase_PHI allocated yes/no
  IF ( ALLOCATED(InData%RRbase_PHI) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! RRbase upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%RRbase_PHI)  ! RRbase
  END IF
  ...
  IF ( .NOT. ALLOCATED(InData%FRIMMP) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FRIMMP,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FRIMMP,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FRIMMP)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FRIMMP))-1 ) = PACK(InData%FRIMMP,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FRIMMP)
  END IF
  IF ( .NOT. ALLOCATED(InData%FMIMMP) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FMIMMP,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FMIMMP,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FMIMMP)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FMIMMP))-1 ) = PACK(InData%FMIMMP,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FMIMMP)
  END IF  
! AÑADIR GIRO
  IF ( .NOT. ALLOCATED(InData%FRIMKP_PHI) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FRIMKP_PHI,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FRIMKP_PHI,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FRIMKP_PHI)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FRIMKP_PHI))-1 ) = PACK(InData%FRIMKP_PHI,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FRIMKP_PHI)
  END IF
  IF ( .NOT. ALLOCATED(InData%FMIMKP_PHI) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FMIMKP_PHI,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FMIMKP_PHI,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FMIMKP_PHI)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FMIMKP_PHI))-1 ) = PACK(InData%FMIMKP_PHI,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FMIMKP_PHI)
  END IF
  IF ( .NOT. ALLOCATED(InData%FRIMCP_PHI) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FRIMCP_PHI,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FRIMCP_PHI,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FRIMCP_PHI)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FRIMCP_PHI))-1 ) = PACK(InData%FRIMCP_PHI,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FRIMCP_PHI)
  END IF
  IF ( .NOT. ALLOCATED(InData%FMIMCP_PHI) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FMIMCP_PHI,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FMIMCP_PHI,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FMIMCP_PHI)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FMIMCP_PHI))-1 ) = PACK(InData%FMIMCP_PHI,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FMIMCP_PHI)
  END IF
  
  IF ( .NOT. ALLOCATED(InData%FRIMMP_PHI) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FRIMMP_PHI,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FRIMMP_PHI,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FRIMMP_PHI)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FRIMMP_PHI))-1 ) = PACK(InData%FRIMMP_PHI,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FRIMMP_PHI)
  END IF
  IF ( .NOT. ALLOCATED(InData%FMIMMP_PHI) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FMIMMP_PHI,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FMIMMP_PHI,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FMIMMP_PHI)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FMIMMP_PHI))-1 ) = PACK(InData%FMIMMP_PHI,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FMIMMP_PHI)
  END IF  
! AÑADIR VERTICAL
  IF ( .NOT. ALLOCATED(InData%FRIMKP_V) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FRIMKP_V,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FRIMKP_V,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FRIMKP_V)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FRIMKP_V))-1 ) = PACK(InData%FRIMKP_V,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FRIMKP_V)
  END IF
  IF ( .NOT. ALLOCATED(InData%FMIMKP_V) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FMIMKP_V,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FMIMKP_V,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FMIMKP_V)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FMIMKP_V))-1 ) = PACK(InData%FMIMKP_V,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FMIMKP_V)
  END IF
  IF ( .NOT. ALLOCATED(InData%FRIMCP_V) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FRIMCP_V,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FRIMCP_V,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FRIMCP_V)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FRIMCP_V))-1 ) = PACK(InData%FRIMCP_V,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FRIMCP_V)
  END IF
  IF ( .NOT. ALLOCATED(InData%FMIMCP_V) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FMIMCP_V,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FMIMCP_V,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FMIMCP_V)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FMIMCP_V))-1 ) = PACK(InData%FMIMCP_V,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FMIMCP_V)
  END IF
  
  IF ( .NOT. ALLOCATED(InData%FRIMMP_V) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FRIMMP_V,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FRIMMP_V,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FRIMMP_V)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FRIMMP_V))-1 ) = PACK(InData%FRIMMP_V,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FRIMMP_V)
  END IF
  IF ( .NOT. ALLOCATED(InData%FMIMMP_V) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FMIMMP_V,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FMIMMP_V,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FMIMMP_V)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FMIMMP_V))-1 ) = PACK(InData%FMIMMP_V,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%FMIMMP_V)
  END IF
! TERMINO VERTICAL
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    DEALLOCATE(mask1)
  END IF  
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FRIMMP not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FRIMMP)) DEALLOCATE(OutData%FRIMMP)
    ALLOCATE(OutData%FRIMMP(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FRIMMP.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FRIMMP)>0) OutData%FRIMMP = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FRIMMP))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FRIMMP)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FMIMMP not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FMIMMP)) DEALLOCATE(OutData%FMIMMP)
    ALLOCATE(OutData%FMIMMP(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FMIMMP.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FMIMMP)>0) OutData%FMIMMP = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FMIMMP))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FMIMMP)
    DEALLOCATE(mask1)
  END IF  
! AÑADIR GIRO
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FRIMKP_PHI not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FRIMKP_PHI)) DEALLOCATE(OutData%FRIMKP_PHI)
    ALLOCATE(OutData%FRIMKP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FRIMKP_PHI.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FRIMKP_PHI)>0) OutData%FRIMKP_PHI = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FRIMKP_PHI))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FRIMKP_PHI)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FMIMKP_PHI not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FMIMKP_PHI)) DEALLOCATE(OutData%FMIMKP_PHI)
    ALLOCATE(OutData%FMIMKP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FMIMKP_PHI.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FMIMKP_PHI)>0) OutData%FMIMKP_PHI = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FMIMKP_PHI))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FMIMKP_PHI)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FRIMCP_PHI not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FRIMCP_PHI)) DEALLOCATE(OutData%FRIMCP_PHI)
    ALLOCATE(OutData%FRIMCP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FRIMCP_PHI.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FRIMCP_PHI)>0) OutData%FRIMCP_PHI = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FRIMCP_PHI))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FRIMCP_PHI)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FMIMCP_PHI not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FMIMCP_PHI)) DEALLOCATE(OutData%FMIMCP_PHI)
    ALLOCATE(OutData%FMIMCP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FMIMCP_PHI.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FMIMCP_PHI)>0) OutData%FMIMCP_PHI = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FMIMCP_PHI))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FMIMCP_PHI)
    DEALLOCATE(mask1)
  END IF  
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FRIMMP_PHI not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FRIMMP_PHI)) DEALLOCATE(OutData%FRIMMP_PHI)
    ALLOCATE(OutData%FRIMMP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FRIMMP_PHI.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FRIMMP_PHI)>0) OutData%FRIMMP_PHI = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FRIMMP_PHI))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FRIMMP_PHI)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FMIMMP_PHI not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FMIMMP_PHI)) DEALLOCATE(OutData%FMIMMP_PHI)
    ALLOCATE(OutData%FMIMMP_PHI(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FMIMMP_PHI.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FMIMMP_PHI)>0) OutData%FMIMMP_PHI = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FMIMMP_PHI))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FMIMMP_PHI)
    DEALLOCATE(mask1)
  END IF  
  ! AÑADIR VERTICAL
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FRIMKP_V not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FRIMKP_V)) DEALLOCATE(OutData%FRIMKP_V)
    ALLOCATE(OutData%FRIMKP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FRIMKP_V.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FRIMKP_V)>0) OutData%FRIMKP_V = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FRIMKP_V))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FRIMKP_V)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FMIMKP_V not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FMIMKP_V)) DEALLOCATE(OutData%FMIMKP_V)
    ALLOCATE(OutData%FMIMKP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FMIMKP_V.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FMIMKP_V)>0) OutData%FMIMKP_V = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FMIMKP_V))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FMIMKP_V)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FRIMCP_V not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FRIMCP_V)) DEALLOCATE(OutData%FRIMCP_V)
    ALLOCATE(OutData%FRIMCP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FRIMCP_V.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FRIMCP_V)>0) OutData%FRIMCP_V = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FRIMCP_V))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FRIMCP_V)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FMIMCP_V not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FMIMCP_V)) DEALLOCATE(OutData%FMIMCP_V)
    ALLOCATE(OutData%FMIMCP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FMIMCP_V.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FMIMCP_V)>0) OutData%FMIMCP_V = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FMIMCP_V))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FMIMCP_V)
    DEALLOCATE(mask1)
  END IF  
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FRIMMP_V not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FRIMMP_V)) DEALLOCATE(OutData%FRIMMP_V)
    ALLOCATE(OutData%FRIMMP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FRIMMP_V.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FRIMMP_V)>0) OutData%FRIMMP_V = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FRIMMP_V))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FRIMMP_V)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FMIMMP_V not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FMIMMP_V)) DEALLOCATE(OutData%FMIMMP_V)
    ALLOCATE(OutData%FMIMMP_V(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FMIMMP_V.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FMIMMP_V)>0) OutData%FMIMMP_V = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FMIMMP_V))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%FMIMMP_V)
    DEALLOCATE(mask1)
  END IF    
  ! TERMINAR VERTICAL
